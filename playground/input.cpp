...
struct Block { double x, y, width, height; Vector speed, acceleration; }; int cornerDx[4] = {1, 1, -1, -1}, cornerDy[4] = {-1, 1, 1, -1};
vector<Block> blocks; Vector speed = {10, 0};
...
void keyboardHandler(unsigned char key, int x, int y)
{
  switch (key)
  {
    case '1':
      blocks.push_back({0});
      for (int i = 0; i < blocks.size(); i ++) blocks[i] = {width * (double)(i + 1) / (blocks.size() + 1), 100, 50, 50, {2, 0}, {0, -gravity}};
      break;
    case '2':
      if (blocks.size()) blocks.pop_back();
      for (int i = 0; i < blocks.size(); i ++) blocks[i] = {width * (double)(i + 1) / (blocks.size() + 1), 100, 50, 50, {2, 0}, {0, -gravity}};
      break;
    default:
      break;
...
void scheduleUpdate(int v)
{
  ...
  { // Force = mass * acceleration
    ...
    if (blocks[i].y < 100) blocks[i].y = 100, blocks[i].speed.y *= -0.5;
    if (blocks[i].y > height - blocks[i].height / 2.0) blocks[i].y = height - blocks[i].height / 2.0, blocks[i].speed.y = 0;
  }
